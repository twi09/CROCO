...
def wachter_rip_recourse(
    ...
    while (f_x_new <= DECISION_THRESHOLD) or (invalidation_rate > invalidation_target + inval_target_eps):
        ...
        # while invalidation_target >= 0.5 and it < n_iter:
            ...
            # invalidation_rate = compute_invalidation_rate(torch_model, random_samples)
            invalidation_rate_c = compute_invalidation_rate_closed(torch_model, x_new_enc, noise_variance)
            ...
            #print(loss_invalidation)
            loss = 3 * loss_invalidation + loss_fn(f_x_new_binary, y_target) + lamb * cost
           
            loss.backward()
            optimizer.step()

            random_samples = reparametrization_trick(x_new_enc, noise_variance,device, n_samples=10000)
            invalidation_rate = compute_invalidation_rate(torch_model, random_samples)
        ...
        #print(loss_invalidation)
        #Rob.append(loss_invalidation.detach().cpu().numpy())
        #Dist.append(cost.detach().cpu().numpy())
    ...
    #np.savetxt("lambda_values_PROB",np.vstack(Lambda))
    #np.savetxt("rob_cost_PROB",np.vstack(Rob))
    #np.savetxt("dist_cost_PROB",np.vstack(Dist))
